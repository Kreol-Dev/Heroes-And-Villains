...
public class MessagesScript : MonoBehaviour
{
	...
	// size of pool
	public int slider = 0;
	//start of shown messages
	public Transform parent;
	//parent of messeges
	public Sprite custom_sprite;
	// sprite of messeges
	public List<InternalMessage> messages = new List<InternalMessage> ();
	// All messeges
	List<ConsoleMessage> messagesPool = new List<ConsoleMessage> ();
	//references to ConsoleMessage
	public List<Category> FilterCat = new List<Category> ();
	// List of all Categories
	public List<InternalMessage.MessageType> FilterType = new List<InternalMessage.MessageType> ();
	// List of all Types
	public List<InternalMessage> ShownMessages = new List<InternalMessage> ();
	//
	public List<InternalMessage.MessageType> ActiveType = new List<InternalMessage.MessageType> ();
	...
	public void RegisterMessage (Category cat, string log, InternalMessage.MessageType type)
	{
	...
	internal void ShowFrom (float value)
	{
		...
		if (ShownMessages.Count < consolePoolSize)
			slider = 0;
		else if (slider > ShownMessages.Count - consolePoolSize)
			slider = ShownMessages.Count - consolePoolSize;
		ShowMessagePool ();
	}

	public bool IsNewType (InternalMessage.MessageType type)
	{
		for (int i = 0; i < FilterType.Count; i++)
		{
			if (type == FilterType [i])
				return false;
		}
		return true;
	}

	public bool IsActiveType (InternalMessage.MessageType type)
	{
		for (int i = 0; i < ActiveType.Count; i++)
		{
			if (type == ActiveType [i])
				return true;
		}
		return false;
	}
