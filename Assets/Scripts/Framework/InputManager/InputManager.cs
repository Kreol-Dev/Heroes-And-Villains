...
public class InputManager : Root
{
    ...
    IEnumerator InputCoroutine ()
    {
        ...
        while (true)
        {
            ...
            //lineRenderer.SetPosition (0, LinePoint (Vector3.zero));
            //lineRenderer.SetPosition (1, LinePoint ((Vector2)Input.mousePosition));
            Hover ((Vector2)Input.mousePosition);
            if (actualDrag)
            {

                //lineRenderer.SetPosition (0, LinePoint (clickStart));
                //lineRenderer.SetPosition (1, LinePoint (clickEnd));
                clickEnd = (Vector2)Input.mousePosition;
                Debug.DrawLine (clickStart, clickEnd);
