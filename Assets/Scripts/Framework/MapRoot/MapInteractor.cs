...
namespace MapRoot
{
	...
	public class MapInteractor : Root
	{
		...
		protected override void CustomSetup ()
		{
			...
			var collections = map.GetAllCollections ();
			ReadInteractors (collections);
			Fulfill.Dispatch ();
		}
		...
		class InteractorBinding
		{
			...
			public InteractorBinding (IMapCollection collection, IMapCollectionInteractor interactor, InteractorState defaultState)
			{
				Interactor = interactor;
				state = defaultState;
				interactor.Setup (collection, defaultState);
			}
		}

		Dictionary<IMapCollection, InteractorBinding> interactors = new Dictionary<IMapCollection, InteractorBinding> ();

		public InteractorState GetCollectionState (IMapCollection collection)
		{
			InteractorBinding interactor;
			if (interactors.TryGetValue (collection, out interactor))
				return interactor.State;
			else
			{
				scribe.LogFormatWarning ("Can't get state of a collection interactor {0} ({1}) - it isn't registered in a states dictionary", collection, collection.GetType ());
				return InteractorState.NotActive;
			}
		}

		public void SetState (IMapCollection collection, InteractorState state)
		{
			InteractorBinding binding = null;
			interactors.TryGetValue (collection, out binding);
			if (binding == null)
			{
				scribe.LogFormatWarning ("No such collection {0}. Interactor state won't be changed", collection.Name);
				return;
			}
			binding.State = state;
		}

		public IMapCollectionInteractor GetInteractor (IMapCollection collection)
		{
			InteractorBinding interactor;
			if (interactors.TryGetValue (collection, out interactor))
			...
			{
				scribe.LogFormatWarning ("Can't get interactor of a collection {0} ({1}) - it isn't registered in interactors dictionary", collection, collection.GetType ());
				return null;
			}
		}

		void ReadInteractors (List<IMapCollection> collections)
		{
			ITable table = Find.Root<ModsManager> ().GetTable ("interactors");
			foreach (var collection in collections)
			{
				string interactorName = collection.Name + "_interactor";
				...
				IMapCollectionInteractor interactor = Activator.CreateInstance (interactorType) as IMapCollectionInteractor;
				if (interactor == null)
				{
				...
				InteractorBinding binding = new InteractorBinding (collection, interactor, state);
				interactors.Add (collection, binding);
